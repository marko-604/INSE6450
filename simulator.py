try:
    ...
    def load_model_from_path(path: str):
        # mujoco expects XML text, not a file path
    ...
    class MjSim:
        ...
        def get_state(self):
            # enough for your current code that stores trajectory
            return self.data
    ...
    class MjViewer:
        def __init__(self, sim):
            self.sim = sim
            # interactive viewer context
            self._ctx = viewer.launch_passive(sim.model, sim.data)

        def render(self):
            # passive viewer updates automatically
            pass
...
class GymSimulation(Simulation):
    def __init__(self, name, total_time=200, recording_time=[0,200]):
        ...
        self.sim = gym.make(name, render_mode="human")
        self.seed_value = 0
        self.reset_seed()
        self.sim.reset()
        self.done = False
    ...
    def run(self, reset=False):
        ...
        self.trajectory = []
        for i in range(self.total_time):
            obs, reward, terminated, truncated, info = self.sim.step(np.array(self.ctrl_array[i]))
    ...
    def seed(self, value=0):
        ...
        self.sim.reset(seed=self.seed_value)
        self.sim.action_space.seed(self.seed_value)
...
class FetchSimulation(Simulation):
    def __init__(self, total_time=152, recording_time=[0,152]):
        ...
        self.sim = gym.make('FetchReachAL-v2')
        self.seed_value = 0
        self.reset_seed()
        self.sim.reset()
        self.done = False
